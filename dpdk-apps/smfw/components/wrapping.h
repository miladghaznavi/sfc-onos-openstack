...
#define SLICE_OFFSET sizeof(struct ether_hdr)
...
wrapper_add_data(struct wrapper_t * this, struct rte_mbuf * packet,
  ...
  packet = rte_pktmbuf_clone(packet, this->mempool);
  ...
  if (slice_offset != offset + packet->data_len) {
    // Slice the packet
    struct rte_mbuf * buf_tail = rte_pktmbuf_alloc(this->mempool);
    *buf_tail = *packet;
  ...
  struct rte_mbuf * buf_metadata = rte_pktmbuf_alloc(this->mempool);
 ...
 */
static void
wrapper_remove_data(struct wrapper_t * this, struct rte_mbuf * packet,
                    struct wrapper_metadata * metadata) {
  const unsigned slice_offset = SLICE_OFFSET;
  ...
  *buf_tail = *packet;
  ...
  rte_memcpy(metadata, rte_pktmbuf_mtod(packet, uint8_t *) + slice_offset, METADATA_LEN);
